...
BEGIN
    ...
    NEW.region := CASE
                      ...
                      WHEN prov_id = 5 OR prov_id = 11 OR prov_id = 16 THEN 'Darién y la comarca Embera Waunán y Wargandí'
...
CREATE TRIGGER tr_rol_updated
    BEFORE UPDATE ON roles
    FOR EACH ROW
EXECUTE FUNCTION set_updated_at();

CREATE TRIGGER tr_permisos_updated
    BEFORE UPDATE ON permisos
    FOR EACH ROW
EXECUTE FUNCTION set_updated_at();

CREATE TRIGGER tr_rol_permisos_updated
    BEFORE UPDATE ON roles_permisos
    FOR EACH ROW
EXECUTE FUNCTION set_updated_at();

CREATE TRIGGER tr_vacunas_updated
    BEFORE UPDATE ON vacunas
    FOR EACH ROW
EXECUTE FUNCTION set_updated_at();

CREATE TRIGGER tr_usuario_rol_updated
    BEFORE UPDATE ON usuarios_roles
    FOR EACH ROW
EXECUTE FUNCTION set_updated_at();

CREATE TRIGGER tr_direcciones_updated
    BEFORE UPDATE ON direcciones
    FOR EACH ROW
EXECUTE FUNCTION set_updated_at();

CREATE TRIGGER tr_dosis_updated
    BEFORE UPDATE ON dosis
    FOR EACH ROW
EXECUTE FUNCTION set_updated_at();

CREATE TRIGGER tr_fabricantes_updated
...
CREATE TRIGGER tr_personas_format_insert
    ...
    FOR EACH ROW EXECUTE FUNCTION fn_personas_format_insert();
...
CREATE TRIGGER tr_pacientes_format_insert
    ...
    FOR EACH ROW EXECUTE FUNCTION fn_pacientes_format_insert();
...
BEGIN
    ...
    IF num_dosis = 'R1' AND NOT EXISTS (
        SELECT 1 FROM dosis
        WHERE paciente = id_paciente
          AND vacuna = id_vacuna
          AND numero_dosis = '1'
    ...
    IF num_dosis = 'R2' AND NOT EXISTS (
        SELECT 1 FROM dosis
        WHERE paciente = id_paciente
          AND vacuna = id_vacuna
...
CREATE TRIGGER tr_validate_dosis_sequence
