 ...
 */
public record ViewPacienteVacunaEnfermedadDto(
        @JsonIgnore @JsonProperty(value = "id_paciente") UUID idPaciente,
        ...
        @Size(max = 13) String dependencia)
        implements Serializable {

    public ViewPacienteVacunaEnfermedadDto(
            String vacuna,
            String numeroDosis,
            String enfermedadesPrevenidas,
            Short edadMinima,
            LocalDateTime fechaAplicacion,
            Double intervaloRecomendado,
            Integer intervaloReal,
            String sede,
            String dependencia,
            UUID idPaciente,
            UUID idVacuna,
            UUID idSede,
            UUID idDosis,
            String idsEnfermedades) {
        this(
                idPaciente,
                idDosis,
                idVacuna,
                vacuna,
                NumDosisEnum.fromValue(numeroDosis.trim().toUpperCase()),
                mapEnfermedades(idsEnfermedades, enfermedadesPrevenidas),
                edadMinima,
                fechaAplicacion,
    ...
    private static List<IdNombreDto> mapEnfermedades(String idsEnfermedades, String enfermedadesPrevenidas) {
        List<Integer> ids = Arrays.stream(idsEnfermedades.split("\\s*,\\s*"))
